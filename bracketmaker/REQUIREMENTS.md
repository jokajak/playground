...
| R10a | Edits survive a redraw | Changing size, layout or any toggle rebuilds the bracket but keeps what you've typed. Every field carries a stable key naming its place in the tree (leaf index / round + first leaf), and state is saved and restored by those keys, so a slot keeps its contents through a rebuild. A value whose slot has gone — an entry past the end of a smaller bracket, a play-in seed's typed name once that seed becomes a dropdown, a quadrant title with its toggle off — is remembered rather than discarded, and comes back when the change is undone; it travels in the save file too, so a reload doesn't lose it either. Clearing a field on screen (or **Clear**) does forget it. Save files written before this (version ≤ 6) stored plain document-order arrays and still load. |
...
| R16 | Quadrant titles (optional) | A **Quadrant titles** toggle. When on, each quadrant gets a free-text name ("East", "Under 12s"…) set on its side down the bracket's outer edge, beside that quadrant's first-round entries — reading bottom-to-top on the left, top-to-bottom on the right, and down the left edge in the vertical layout. Titles are saved, loaded and printed like every other field; a blank one prints as a rule to letter in by hand. Disabled for a 2-bracket, which is a single match with no quadrants. Off by default and stored under new keys (`q0`..`q3`), so brackets saved before it existed load unchanged, and a file saved with titles still loads in an older copy of the page. |
